...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Commands::CheckContactTypeCompanyGivenName { fix, dry_run } => commands::cmd_check_contact_type_company_given_name(fix, dry_run).await?,
        Commands::CheckContactTypeCompanyNoLabel { fix, dry_run } => commands::cmd_check_contact_type_company_no_label(fix, dry_run).await?,
